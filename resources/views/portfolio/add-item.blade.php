...
$selected = old('item_type', 'project');
@endphp
...
<div style="max-width:700px;background:rgba(255,255,255,.03);border:1.5px solid rgba(245,240,232,.1);border-radius:.75rem;padding:30px">
    <form method="POST" action="{{ route('dashboard.items.store') }}" enctype="multipart/form-data">
        ...
        <div style="margin-bottom:1.5rem">
            ...
            <input type="hidden" name="item_type" id="item_type" value="{{ $selected }}" required>
        ...
        <div style="margin-bottom:1.2rem">
            ...
            <input type="text" name="title" value="{{ old('title') }}" required maxlength="255" class="field-input">
            @error('title')<p class="field-error">{{ $message }}</p>@enderror
        </div>
        <div style="margin-bottom:1.2rem">
            <label class="field-label">Description</label>
            <textarea name="description" rows="4" required class="field-input" style="resize:vertical">{{ old('description') }}</textarea>
            @error('description')<p class="field-error">{{ $message }}</p>@enderror
        </div>
        <div style="margin-bottom:1.2rem">
            <label class="field-label">Date <span class="field-hint">(optional)</span></label>
            <input type="date" name="item_date" value="{{ old('item_date') }}" class="field-input">
        </div>
        <div style="margin-bottom:1.2rem">
            <label class="field-label">Tags <span class="field-hint">(comma-separated)</span></label>
            <input type="text" name="tags_input" value="{{ old('tags_input') }}" placeholder="PHP, Laravel, MySQL" class="field-input">
        </div>
        <div style="margin-bottom:1.8rem">
            <label class="field-label">Image <span class="field-hint">(optional, max 10MB)</span></label>
